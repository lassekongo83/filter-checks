...
def check_redirect(domain):
 ...
 try:
  ...
  if response.url != url:
      ...
      if initial_domain != final_domain and not final_domain.startswith(f'www.{initial_domain}') and not final_domain.endswith(':443/') and not final_domain.endswith(':443'):
